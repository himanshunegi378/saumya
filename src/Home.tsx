...
const MaterialCard = ({ onSelectMaterial, dummyDatas }: any) => {
  ...
  const toggleSelectMaterial = (materialId: any, materialName: any) => {
    ...
    const allStudies = materialIds
      .map(
        (id) =>
          dummy.materials.find((material: any) => material.id === id)
            ?.studies || []
      )
      .flat();
          ...
          {filteredMaterials.map((material: any) => (
            ...
            >
              <Heading
                size="sm"
                color={
                  selectedMaterialIds.includes(material.id) ? "white" : "black"
                }
              >
                {material.name}
              </Heading>
...
const StudyCard = ({ selectedMaterial, onSelectStudies }: any) => {
  ...
  const filteredStudies = dummyData.studies.filter(
    (study: any) =>
      selectedMaterial &&
      selectedMaterial.includes(study.id) &&
  ...
  const toggleSelectStudies = (materialId: any, materialName: any) => {
    ...
    const allStudies = materialIds
      .map(
        (id) =>
          dummyData.studies.find((material: any) => material.id === id)
            ?.protocols || []
      )
      .flat();
    // console.log(allStudies, "allStudies......");
...
const Protocols = ({ selectedStudies, onSelectProtocols }: any) => {
  ...
  const toggleSelectProtocols = (materialId: any, materialName: any) => {
    ...
    const allStudies = materialIds
      .map(
        (id) =>
          dummyData.protocols.find((material: any) => material.id === id)
            ?.tests || []
      )
      .flat();
    //  console.log(allStudies,"allStudies......")
        ...
        <Stack divider={<StackDivider />} spacing="4">
          {filteredStudies.map((material: any) => (
            ...
            >
              <Heading
                size="sm"
                color={selectedMaterialId === material.id ? "white" : "black"}
              >
                {material.name}
              </Heading>
...
const Resources = ({
  ...
  const handleAllocate = async (selectedMaterialId: any) => {
    ...
    try {
      ...
      const lastAllocation = resources.filter(
        (resource: any) => resource.Protocols && resource.Protocols.includes(selProtocols)
      ) as { startDate: Date , duration: string, endDate: string }[];
      
      // console.log("dates",startDates)
      console.log("dui dui di",lastAllocation);
      if (lastAllocation.length > 0 && lastAllocation[0].endDate) {
        startDate = new Date(lastAllocation[0].endDate);
      }
  

       startDate = lastAllocation
        ? new Date(startDate)
        : new Date();
                ...
                >
                  ...
                  >
                    <Heading
                      size="sm"
                      color={
                        selectedMaterialId === material.id ? "white" : "black"
                      }
                    >
                      {material.name}
                    </Heading>
...
function Home() {
    ...
    const dummyData = async () => {
      try {
        const materialsResponse = await fetch(
          "https://658d4ccc7c48dce94738dde6.mockapi.io/materials/materials"
        );
  ...
  const onSelectMaterial = (
    material: SetStateAction<null>,
    selectedMaterial: SetStateAction<null>
  ) => {
  ...
  const backgroundImage = 'https://img.freepik.com/free-photo/3d-geometric-abstract-cuboid-wallpaper-background_1048-9891.jpg?w=1380&t=st=1705381885~exp=1705382485~hmac=5a84f5b4bdcc6fa46553c7a951fab203766993ae664bf1d122773e4f7210d610';

  return (
    <Box
  // backgroundImage={`url(${backgroundImage})`}
  // bgGradient="linear-gradient(to bottom, white, grey)"
  backgroundSize="cover"
  backgroundPosition="center"
  backgroundRepeat="no-repeat"
  minHeight="100vh" // Adjust this to cover the full height of the viewport
>
      <Box
        p={4}
